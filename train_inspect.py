...
def load_config():
    ...
    parser.add_argument("--end_idx", type=int, default=5, help="end index for iteration")
...
def main():
    ...
    for img, points, gt_tour, sample_idx, constraint in tqdm_partial(test_dataloader):
        ...
        xT = torch.randn_like(img).to(device).half()
        model = Model_x0(
            batch_size=config.batch_size_sample,
        ...
        if config.constraint_type == 'basic':
            ...
            points, gt_tour, constraint = points.cpu().numpy()[0], gt_tour.cpu().numpy()[0], constraint.cpu().numpy()[0]
        ...
        for epoch in range(config.num_epochs):
            ...
            for _ in range(num_sample): # index 없애도 되는지 check
                if epoch == 0:
                    model.reset()
