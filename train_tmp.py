...
def main(_):
    ...
    if config.train.use_8bit_adam: # TODO: need to change
        try:
    ...
    model = Model_x0(
        ...
        line_thickness = config.image.line_thickness,).to(accelerator.device) #TODO: check batch_size from sample vs train
    ########### add prior model ###########
    ...
    neg_prompt_embed = pipeline.text_encoder(
        pipeline.tokenizer(
            ["TSP"], # [""], TODO: need to descript problem definition 
            return_tensors="pt",
            padding="max_length",
            truncation=True,
    ...
    model, optimizer = accelerator.prepare(model, optimizer)    # unet, optimizer = accelerator.prepare(unet, optimizer)
    ...
    test_dataset = TSPDataset(data_file=f'./data/tsp50_test_concorde.txt',
                              ...
                              line_color = config.image.line_color)
    test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=config.sample.batch_size, shuffle=True)
    print('Created dataset')
    
    '''
    task1. only one point
    '''
    img, sample_idx = next(iter(test_dataloader))
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for i in tqdm(
            ...
            # prompt_embeds = pipeline.text_encoder(prompt_ids)[0] # check why this line require reward part
            prompt_embeds = torch.load('./data/prompt_embeds')
            # sample
            with autocast():
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            for i, image in enumerate(images):
                ...
                # )
                pil = Image.fromarray((image[0].cpu().numpy() * 255).astype(np.uint8), 'L')
        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            for i, sample in tqdm(
                ...
                for j in tqdm(
                    ...
                    if accelerator.sync_gradients: #TODO: check the meaning
