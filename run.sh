...
available_devices=(0 1 2 3 4)  # Specify the GPU IDs you want to use
...
for (( start_idx=0; start_idx+interval<=1280; start_idx+=interval )); do
    ...
    CUDA_VISIBLE_DEVICES=$device_number nohup python train.py --run_name $run_name --start_idx $start_idx --end_idx $end_idx --num_cities $num_cities --num_epochs $num_epochs --num_inner_epochs $num_inner_epochs --num_init_sample $num_init_sample\
